...
def init_csv(first_question, mode_label):
    ...
    csv_writer.writerow(['timestamp', 'mode', 'question', 'primary_hex', 'relating_hex', 'changing_lines', 'line_values', 'bit_position', 'mercury', 'venus', 'mars', 'jupiter', 'saturn'])
    csv_file_handle.flush()
    print(f"  Logging to: {filename}\n")

def log_cast(mode_label, question, primary_num, secondary_num, lines, bit_start=None, planets=None):
    ...
    csv_writer.writerow([
        ...
        planets.get('mercury','') if planets else "",
        planets.get('venus','') if planets else "",
        planets.get('mars','') if planets else "",
        planets.get('jupiter','') if planets else "",
        planets.get('saturn','') if planets else "",
    ])
    csv_file_handle.flush()
...
def load_bits(path="ghost_seed_combined.txt"):
...
def do_cast(question=None, mode_label="", planets=None):
    bit_start = bit_index
    lines = cast_hexagram()
    ...
    if has_changes:
        ...
        log_cast(mode_label, question, primary_num, secondary_num, lines, bit_start, planets)
    else:
        print(f"\n  Hexagram {primary_num} is stable. No changing lines.\n")
        log_cast(mode_label, question, primary_num, None, lines, bit_start, planets)


def get_planet_degrees():
    """Return whole-degree ecliptic longitudes for the five visible planets."""
    now = ephem.now()
    planets = {
    ...
    return {name: int(math.degrees(p.hlong)) % 360
            for name, p in planets.items()}

def question_to_offset(question):
    """Hash question + current planetary positions to get bit stream offset."""
    degrees = get_planet_degrees()
    planet_str = "-".join(str(degrees[p]) for p in ['mercury','venus','mars','jupiter','saturn'])
    seed = f"{question.strip().lower()}|{planet_str}"
    h = hashlib.sha256(seed.encode()).digest()
    return int.from_bytes(h[:4], 'big') % len(bits), degrees

# ── MAIN ──────────────────────────────────────────────────────────────────────

if __name__ == "__main__":
    print("\nLoading conversation history...")
    bits = load_bits("ghost_seed_combined.txt")
        ...
        while True:
            ...
            if question:
                ...
                if not csv_initialized:
                    ...
                    csv_initialized = True
                pos, degrees = question_to_offset(current_question)
                set_position(pos)
                print(f"  Planets: " + ", ".join(f"{k}={v}°" for k,v in degrees.items()))
            ...
            input("  Press Enter to cast...")
            do_cast(current_question, mode_label, planets=degrees if question else None)
